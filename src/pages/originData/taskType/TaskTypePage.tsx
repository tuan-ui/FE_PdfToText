...
import CreateOrUpdateTaskType from './CreateOrUpdateTaskType';
import { useMenuPermission } from '../../../hooks/useMenuPermission';

const TaskTypePage: React.FC = () => {
  ...
  const perm = useMenuPermission('TASKTYPE');
  const handleClearAllFilters = () => {
    try {
      (taskTypeCodeSearch as any)?.resetSearch?.();
      setSearchParams({});
      fetchTaskTypes(pagination.current, pagination.pageSize);
    } catch (e) {
      /* ignore */
    }
    try {
      (taskTypeNameSearch as any)?.resetSearch?.();
    } catch (e) {
      /* ignore */
    }
    try {
      (taskTypeDesSearch as any)?.resetSearch?.();
    } catch (e) {
      /* ignore */
    }
    ...
    fetchTaskTypes(1, pagination.pageSize, {});
  };

  const taskTypeNameSearch = useColumnSearch<TaskType>({
    dataIndex: 'taskTypeName',
    onSearchServer: (field, value) => {
      setSearchParams((prev) => {
        const newParams = { ...prev, [field]: value };
        fetchTaskTypes(1, pagination.pageSize, newParams);
        return newParams;
      });
    },
  });

  const taskTypeCodeSearch = useColumnSearch<TaskType>({
  ...
  const taskTypeDesSearch = useColumnSearch<TaskType>({
    dataIndex: 'taskTypeDescription',
    onSearchServer: (field, value) => {
      setSearchParams((prev) => {
        const newParams = { ...prev, [field]: value };
        fetchTaskTypes(1, pagination.pageSize, newParams);
  ...
  const fetchTaskTypes = async (
    ...
    try {
      ...
      if (res?.status === 200 && res?.object) {
        setTaskTypes(res.object.content || []);
  ...
  const handleTableChange = (paginationInfo: any) => {
    ...
    fetchTaskTypes(current, pageSize, activeFilters);
  };
  /** Hàm xóa */
  const handleDelete = async (record: TaskType) => {
    await perm.exec('delete', async () => {
      try {
        if (record.id) {
          const res = await deleteTaskType(record.id, record.version);
          if (res.success) {
            if (res?.message?.data?.status === 200) {
              notification.success({
                message: t('common.actionSuccess'),
                description: `${t('common.Delete')} ${record.taskTypeName} ${t(
                  'common.success'
                )}`,
              });
              fetchTaskTypes(pagination.current, pagination.pageSize);
  ...
  /** Hàm đổi trạng thái */
  const handleStatusClick = async (record: TaskType) => {
    await perm.exec('edit', async () => {
      try {
        if (record.id) {
          const res = await lockTaskType(record.id, record.version);
          if (res.success) {
            if (res?.message?.data?.status === 200) {
              notification.success({
                message: t('common.actionSuccess'),
        ...
        notification.error({
          message: t('common.actionFailed'),
          description: `${t('common.changeStatus')} ${record.taskTypeName} ${t(
            'common.failed'
          )}`,
        });
      }
  ...
  const handleAdd = async (record: TaskType) => {
    const success = await perm.exec(editTaskType ? 'edit' : 'add', async () => {
      try {
        const res = editTaskType
          ? await updateTaskType(record)
          : await createTaskType(record);

        if (res.status === 200) {
          notification.success({
            message: t('common.actionSuccess'),
            description: editTaskType
  ...
  const handleDeleteSelected = async () => {
    ...
    const selectedRecords = taskTypes.filter((taskTypes) =>
      selectedRowKeys.includes(taskTypes.id!)
    );

    const itemsToDelete = selectedRecords.map((record) => ({
      id: String(record.id),
      name: record.taskTypeName,
      code: record.taskTypeCode,
    ...
    try {
      ...
      if (!hasError && payload === null) {
        const deleteRes = await deleteMultiTaskType(itemsToDelete);
      ...
      setErrorDeleteUrl('/api/task-type/deleteMuti');
  ...
  const COLUMNS: ColumnsType<TaskType> = [
    {
      title: t('originData.taskType.code'),
      dataIndex: 'taskTypeCode',
      key: 'taskTypeCode',
      align: 'left',
      width: '20%',
      ellipsis: true,
      ...taskTypeCodeSearch.getColumnSearchProps(),
    },
    {
      title: t('originData.taskType.name'),
      dataIndex: 'taskTypeName',
      key: 'taskTypeName',
      align: 'left',
      width: '20%',
      ellipsis: true,
      ...taskTypeNameSearch.getColumnSearchProps(),
    },
    {
      title: t('originData.taskType.description'),
      dataIndex: 'taskTypeDescription',
      key: 'taskTypeDescription',
      align: 'left',
      width: '20%',
      ellipsis: true,
      ...taskTypeDesSearch.getColumnSearchProps(),
    },
    {
      title: t('originData.taskType.priority'),
      dataIndex: 'taskTypePriority',
      key: 'taskTypePriority',
      align: 'left',
      width: '20%',
      ...taskTypeDesSearch.getColumnSearchProps(),
      render: (value) => {
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
          ...
          {perm.canDelete && (
            <Tooltip title={t('common.Delete')}>
              <Popconfirm
                placement="topRight"
                title={t('app.ConfirmDelete', { name: record.taskTypeName })}
                okText={t('common.yes')}
  ...
  return (
    <div>
          ...
          <Card>
            <>
              ...
              >
                <Col>
                  <Helmet>
                    <title>{t('originData.taskType.list')}</title>
                  </Helmet>

                  <PageHeader
                    title={t('originData.taskType.list')}
      ...
      />
      <CreateOrUpdateTaskType
        open={openAddModal}
        onClose={handleCloseAdd}
        onSubmit={handleAdd}
        taskTypeData={editTaskType}
      />
